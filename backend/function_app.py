    ...
    for email_data in emails:
        ...
        try:
            if not email.get("hasAttachments"):
                logging.info(f"No attachments found in email: {subject}")
                pdf_bytes = convert_email_to_pdf(email)
                ...
                logging.info(f"Uploaded email body PDF: {blob_name}")
            ...
            logging.error(
                f"Failed to process email '{subject}': {str(e)}", exc_info=True)
            continue
